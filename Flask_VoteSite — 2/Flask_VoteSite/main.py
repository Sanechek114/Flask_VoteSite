...
def create_app():
    ...
    def register():
        ...
        if form.validate_on_submit():
            ...
            flash('Регистрация успешна' + (' Вы — администратор!' if is_first_user else ''))
    ...
    @admin_required
    def delete_user(id):
        if id == current_user.id: flash('Нельзя удалить себя'); return redirect(url_for('admin'))
        u = User.query.get_or_404(id)
        db.session.delete(u); db.session.commit()
        flash('Пользователь удалён'); return redirect(url_for('admin'))
    ...
    def delete_poll(id):
        ...
        flash('Опрос удалён'); return redirect(url_for('admin'))

    # --- Создание опроса (2 шага) ---
    ...
    def create_options():
        ...
        class TempPollForm(FlaskForm): pass
        for i in range(num):
            setattr(TempPollForm, f'opt{i}', StringField(f'Вариант {i+1}', validators=[DataRequired()]))
        ...
        if form.validate_on_submit():
            ...
            for i in range(num):
                db.session.add(Option(poll_id=p.id, text=getattr(form, f'opt{i}').data))
            db.session.commit()
            session.pop('poll_title', None); session.pop('poll_num', None)
    ...
    def vote_web(id):
        ...
        if not opt_id: return redirect(url_for('poll', id=id))
